...
====================================================[ BOTZ RUIN FF ]==============================================================
SUBSCRIBE RUIN FFSUBSCRIBE RUIN FF
SUBSCRIBE RUIN FFSUBSCRIBE RUIN FF
SUBSCRIBE RUIN FFSUBSCRIBE RUIN FF
SUBSCRIBE RUIN FFSUBSCRIBE RUIN FF
SUBSCRIBE RUIN FFSUBSCRIBE RUIN FF
...
SUBSCRIBE RUIN FFSUBSCRIBE RUIN FF
SUBSCRIBE RUIN FFSUBSCRIBE RUIN FF
SUBSCRIBE RUIN FF SUBSCRIBE RUIN FF 
*/
const { 
...
let sewaa3 = 'SEWA + PREMIUM 15K';  
let sewaa4 = 'PERMANEN 25K';    
let sewaa5 = 'PERMANEN + PREM 30K';  
let sewaa6 = 'ALL PERMANEN 35K';  
//sewa price list 2.    
let sewa1 = 'JADI BOT 10K/BLN';     
let sewa2 = 'JADI BOT + OWNER 30K';
let sewa3 = 'SEWA + PREMIUM 15K';  
...
////< DAFTAR BUTTON BY Created Subscribe RUIN FF > ////
...
sendKontak(from, `${owner}`, `${ownerName}`, 'Subscribe RUIN FF!!')
sendKontak(from, `6285855159230`, `Developer`, 'Sv Ardi kak')
await sleep(1000)
...
case 'next': case 'leave': case "stop": {
  ...
  if (!room) {
    await Lazuardi.sendButton(from, '_Kamu tidak sedang berada di anonymous chat_', `${Lazuardi.user.name}`, 'Cari Partner', prefix+'start')
    return false
  }
  ...
  case 'mulai': case 'start': case "anonymous": case "mutual": {
    ...
    if (Object.values(Lazuardi.anonymous).find(room => room.check(sender))) {
      await kyy.sendButton(from, '_Kamu masih berada di dalam anonymous chat, menunggu partner_', `${Lazuardi.user.name}`, 'Keluar', `${prefix}leave`)
      return false
    }
    let room = Object.values(Lazuardi.anonymous).find(room => room.state === 'WAITING' && !room.check(sender))
    if (room) {
      await Lazuardi.sendButton(room.a, '_Partner ditemukan!_', `${Lazuardi.user.name}`, 'Next', prefix+'next')
      room.b = sender
      room.state = 'CHATTING'
      await Lazuardi.sendButton(room.b, '_Partner ditemukan!_', `${Lazuardi.user.name}`, 'Next', prefix+'next')
    } else {
      let id = + new Date
      ...
      await Lazuardi.sendButton(from, '_Menunggu partner..._', `${Lazuardi.user.name}`, 'Keluar', prefix+'leave')
    }
    break
  }
           case 'bucinstick':
...
Case By Iky`;
        	var imgs = await Lazuardi.prepareMessage('0@c.us', fakeimage, image, { thumbnail: fakeimage })
            var imgCatalog = imgs.message.imageMessage
            var ctlg = await Lazuardi.prepareMessageFromContent(from, {
                "productMessage": {
                    "product": {
                        "productImage": imgCatalog,
                        "productId": "4715716298440452",
                        "title": `ANONYMOUS BOT BETA`,
                        "description": menu,
                        "footerText": `�RUIN FF`,
                        "currencyCode": "IDR",
                        ...
                        "retailerId": `SUBSCRIBE : RUIN FF`,
                        "url": "https://instabio.cc/ruin999"
                    },
                    "businessOwnerJid": "6285855159153@s.whatsapp.net",
                }
            }, { quoted: freply, mimetype: 'image/jpeg' })
            Lazuardi.relayWAMessage(ctlg)
            break
...
▹ ${prefix} wallgravity text1 text2
 `
sendButMessage(from, wibu, `Created Subscribe RUIN FF`, [
          {
...
`
   sendButMessage(from, wibu, `Created Subscribe RUIN FF`, [
        ...
        case 'simi':
              ...
              buttonsMessage = {footerText:'Created Subscribe RUIN FF', imageMessage: imageMsg,
...
▹ ${prefix} bokep25
 `
               ...
               sendButMessage(from, menu, `Created Subscribe RUIN FF`, [
          {
            buttonId: noprefix ? "menu" : "!"+"menu",
            buttonText: {
              displayText: `⬅️ BACK TO MENU `,
...
`
   ...
   case 'command':
               list = []
               listmenu = [`sewabot`,`groupmenu`,`photoxy`,`ephoto`,`randomimage`,`wibu2`,`storymenu`,`porno`,`randomtext`,`islammenu`,`wibumenu`,`stickermenu`,`ownermenu`,`gamemenu`,`funmenu`,`downloadmenu`,`infomenu`,`othermenu`,`owner`,`Lazuardigroup`]
               listmenuu = [`Sewa/Premium`,`Menu Group`,`Photo Oky`,`Ephoto Menu`,`Random Image`,`Nsfw Anime`,`Movie&Story`,`18+ Menu`,`RandomText`,`Islam Menu`,`Wibu Menu`,`Sticker Menu`,`Owner Command`,`Game Menu`,`For Fun Menu`,`Downloader`,`Info Menu`,`Menu Lainnya`,`OwnerBot`,`Official Group`]
               nombor = 1
               startnum = 0
...
case 'store':
               list = []
               listmenu = [`topupff`,`topuppubg`,`topupml`]
               listmenuu = [`FREE FIRE`,`PUBG MOBILE`,`MOBILE LEGENDS`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
               const yy = {title: 'menu ke' + nombor++,
...
List Top Up Game Di Sini KK Silahkan Dipilih
`,`🐣 Day : ${week} ${weton}\n🐥 Date : ${date}              `, list, { quoted: freply})
               break
case 'topupff':
              gopeynya = `${ppbot}`
            buff = await getBuffer(gopeynya)
            teksnya = `*${ucapanWaktu} ${pushname} 👋*
            
Brikut List TopUp Free Fire Yang Tersedia Di ${botName}
`
  buttons = [{buttonId: noprefix ? "topupff1" : "!"+"topupff1",buttonText:{displayText: `📝LIST DIAMOND`},type:1},{buttonId: `${prefix}member`,buttonText:{displayText: `💳MEMBERSHIP`},type:1}]
              imageMsg = (await Lazuardi.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created Subscribe RUIN FF', imageMessage: imageMsg,
...
Brikut List TopUp PUBG MOBILE Yang Tersedia Di ${botName}
`
  buttons = [{buttonId: noprefix ? "topuppubg1" : "!"+"topuppubg1",buttonText:{displayText: `📝LIST UC`},type:1}]
              imageMsg = (await Lazuardi.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created Subscribe RUIN FF', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await Lazuardi.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
...
Brikut List TopUp MOBILE LEGENDS Yang Tersedia Di ${botName}
`
  buttons = [{buttonId: noprefix ? "topupml1" : "!"+"topupml1",buttonText:{displayText: `📝LIST DIAMOND`},type:1},{buttonId: `${prefix}star`,buttonText:{displayText: `💳STARLIGHT`},type:1}]
              imageMsg = (await Lazuardi.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created Subscribe RUIN FF', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await Lazuardi.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
...
case 'topupff1':
               list = []
               listmenu = [`pay1`,`pay2`,`pay3`,`pay4`,`pay5`,`pay6`,`pay7`,`pay8`,`pay9`,`pay10`,`pay11`]
               listmenuu = [`${_diamond1} = ${diamond1}`,`${_diamond2} = ${diamond2}`,`${_diamond3} = ${diamond3}`,`${_diamond4} = ${diamond4}`,`${_diamond5} = ${diamond5}`,`${_diamond6} = ${diamond6}`,`${_diamond7} = ${diamond7}`,`${_diamond8} = ${diamond8}`,`${_diamond9} = ${diamond9}`,`${_diamond10} = ${diamond10}`,`${_diamond11} = ${diamond11}`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
               const yy = {title: 'menu ke' + nombor++,
...
List Diamond Di Sini Kak Silahkan Dipilih
`,`🐣 Day : ${week} ${weton}\n🐥 Date : ${date}              `, list, { quoted: freply})
               break
case 'topuppubg1':
               list = []
               listmenu = [`payuc1`,`payuc2`,`payuc3`,`payuc4`,`payuc5`,`payuc6`,`payuc7`,`payuc8`,`payuc9`,`payuc10`,`payuc11`,`payuc12`,`payuc13`]
               listmenuu = [`${_uc1} = ${uc1}`,`${_uc2} = ${uc2}`,`${_uc3} = ${uc3}`,`${_uc4} = ${uc4}`,`${_uc5} = ${uc5}`,`${_uc6} = ${uc6}`,`${_uc7} = ${uc7}`,`${_uc8} = ${uc8}`,`${_uc9} = ${uc9}`,`${_uc10} = ${uc10}`,`${_uc11} = ${uc11}`,`${_uc12} = ${uc12}`,`${_uc13} = ${uc13}`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
               const yy = {title: 'menu ke' + nombor++,
...
List UC Di Sini Kak Silahkan Dipilih
`,`🐣 Day : ${week} ${weton}\n🐥 Date : ${date}              `, list, { quoted: freply})
               break
case 'topupml1':
               list = []
               listmenu = [`payml1`,`payml2`,`payml3`,`payml4`,`payml5`,`payml6`,`payml7`,`payml8`,`payml9`,`payml10`,`payml11`]
               listmenuu = [`${_ml1} = ${ml1}`,`${_ml2} = ${ml2}`,`${_ml3} = ${ml3}`,`${_ml4} = ${ml4}`,`${_ml5} = ${ml5}`,`${_ml6} = ${ml6}`,`${_ml7} = ${ml7}`,`${_ml8} = ${ml8}`,`${_ml9} = ${ml9}`,`${_ml10} = ${ml10}`,`${_ml11} = ${ml11}`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
               const yy = {title: 'menu ke' + nombor++,
...
case 'member':
               list = []
               listmenu = [`pay12`,`pay13`]
               listmenuu = [`${_diamond12} = ${diamond12}`,`${_diamond13} = ${diamond13}`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
               const yy = {title: 'menu ke' + nombor++,
...
List Membership Di Sini Kak Silahkan Dipilih
`,`🐣 Day : ${week} ${weton}\n🐥 Date : ${date}              `, list, { quoted: freply})
               break
case 'star':
               list = []
               listmenu = [`payml12`,`payml13`]
               listmenuu = [`${_ml12} = ${ml12}`,`${_ml13} = ${ml13}`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
               const yy = {title: 'menu ke' + nombor++,
...
List Starlight Di Sini Kak Silahkan Dipilih
`,`🐣 Day : ${week} ${weton}\n🐥 Date : ${date}              `, list, { quoted: freply})
               break
///pyment nya guys 
//PUBGGGGG
...
• Pilih Payment Jika Ingin Membeli ${_uc1} Dengan Harga ${uc1}

• Pilih Back Jika Ingin Kembali Ke List Menu`
  buttons = [{buttonId: noprefix ? "store" : "!"+"store",buttonText:{displayText: `⬅️BACK`},type:1},{buttonId: `${prefix}ppubg1`,buttonText:{displayText: `💸PAYMENT`},type:1}]
              imageMsg = (await Lazuardi.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created Subscribe RUIN FF', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await Lazuardi.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
...
• Pilih Payment Jika Ingin Membeli ${_uc2} Dengan Harga ${uc2}

• Pilih Back Jika Ingin Kembali Ke List Menu`
  buttons = [{buttonId: noprefix ? "store" : "!"+"store",buttonText:{displayText: `⬅️BACK`},type:1},{buttonId: `${prefix}ppubg2`,buttonText:{displayText: `💸PAYMENT`},type:1}]
              imageMsg = (await Lazuardi.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created Subscribe RUIN FF', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await Lazuardi.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
...
• Pilih Payment Jika Ingin Membeli ${_uc3} Dengan Harga ${uc3}

• Pilih Back Jika Ingin Kembali Ke List Menu`
  buttons = [{buttonId: noprefix ? "store" : "!"+"store",buttonText:{displayText: `⬅️BACK`},type:1},{buttonId: `${prefix}ppubg3`,buttonText:{displayText: `💸PAYMENT`},type:1}]
              imageMsg = (await Lazuardi.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created Subscribe RUIN FF', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await Lazuardi.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
...
• Pilih Payment Jika Ingin Membeli ${_uc4} Dengan Harga ${uc4}

• Pilih Back Jika Ingin Kembali Ke List Menu`
  buttons = [{buttonId: noprefix ? "store" : "!"+"store",buttonText:{displayText: `⬅️BACK`},type:1},{buttonId: `${prefix}ppubg4`,buttonText:{displayText: `💸PAYMENT`},type:1}]
              imageMsg = (await Lazuardi.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created Subscribe RUIN FF', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await Lazuardi.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
...
• Pilih Payment Jika Ingin Membeli ${_uc5} Dengan Harga ${uc5}

• Pilih Back Jika Ingin Kembali Ke List Menu`
  buttons = [{buttonId: noprefix ? "store" : "!"+"store",buttonText:{displayText: `⬅️BACK`},type:1},{buttonId: `${prefix}ppubg5`,buttonText:{displayText: `💸PAYMENT`},type:1}]
              imageMsg = (await Lazuardi.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created Subscribe RUIN FF', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await Lazuardi.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
...
• Pilih Payment Jika Ingin Membeli ${_uc6} Dengan Harga ${uc6}

• Pilih Back Jika Ingin Kembali Ke List Menu`
  buttons = [{buttonId: noprefix ? "store" : "!"+"store",buttonText:{displayText: `⬅️BACK`},type:1},{buttonId: `${prefix}ppubg6`,buttonText:{displayText: `💸PAYMENT`},type:1}]
              imageMsg = (await Lazuardi.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created Subscribe RUIN FF', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await Lazuardi.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
...
• Pilih Payment Jika Ingin Membeli ${_uc7} Dengan Harga ${uc7}

• Pilih Back Jika Ingin Kembali Ke List Menu`
  buttons = [{buttonId: noprefix ? "store" : "!"+"store",buttonText:{displayText: `⬅️BACK`},type:1},{buttonId: `${prefix}ppubg7`,buttonText:{displayText: `💸PAYMENT`},type:1}]
              imageMsg = (await Lazuardi.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created Subscribe RUIN FF', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await Lazuardi.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
...
• Pilih Payment Jika Ingin Membeli ${_uc8} Dengan Harga ${uc8}

• Pilih Back Jika Ingin Kembali Ke List Menu`
  buttons = [{buttonId: noprefix ? "store" : "!"+"store",buttonText:{displayText: `⬅️BACK`},type:1},{buttonId: `${prefix}ppubg8`,buttonText:{displayText: `💸PAYMENT`},type:1}]
              imageMsg = (await Lazuardi.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created Subscribe RUIN FF', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await Lazuardi.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
...
• Pilih Payment Jika Ingin Membeli ${_uc9} Dengan Harga ${uc9}

• Pilih Back Jika Ingin Kembali Ke List Menu`
  buttons = [{buttonId: noprefix ? "store" : "!"+"store",buttonText:{displayText: `⬅️BACK`},type:1},{buttonId: `${prefix}ppubg9`,buttonText:{displayText: `💸PAYMENT`},type:1}]
              imageMsg = (await Lazuardi.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created Subscribe RUIN FF', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await Lazuardi.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
...
• Pilih Payment Jika Ingin Membeli ${_uc10} Dengan Harga ${uc10}

• Pilih Back Jika Ingin Kembali Ke List Menu`
  buttons = [{buttonId: noprefix ? "store" : "!"+"store",buttonText:{displayText: `⬅️BACK`},type:1},{buttonId: `${prefix}ppubg10`,buttonText:{displayText: `💸PAYMENT`},type:1}]
              imageMsg = (await Lazuardi.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created Subscribe RUIN FF', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await Lazuardi.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
...
• Pilih Payment Jika Ingin Membeli ${_uc11} Dengan Harga ${uc11}

• Pilih Back Jika Ingin Kembali Ke List Menu`
  buttons = [{buttonId: noprefix ? "store" : "!"+"store",buttonText:{displayText: `⬅️BACK`},type:1},{buttonId: `${prefix}ppubg11`,buttonText:{displayText: `💸PAYMENT`},type:1}]
              imageMsg = (await Lazuardi.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created Subscribe RUIN FF', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await Lazuardi.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
...
• Pilih Payment Jika Ingin Membeli ${_uc12} Dengan Harga ${uc12}

• Pilih Back Jika Ingin Kembali Ke List Menu`
  buttons = [{buttonId: noprefix ? "store" : "!"+"store",buttonText:{displayText: `⬅️BACK`},type:1},{buttonId: `${prefix}ppubg12`,buttonText:{displayText: `💸PAYMENT`},type:1}]
              imageMsg = (await Lazuardi.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created Subscribe RUIN FF', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await Lazuardi.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
...
• Pilih Payment Jika Ingin Membeli ${_uc13} Dengan Harga ${uc13}

• Pilih Back Jika Ingin Kembali Ke List Menu`
  buttons = [{buttonId: noprefix ? "store" : "!"+"store",buttonText:{displayText: `⬅️BACK`},type:1},{buttonId: `${prefix}ppubg13`,buttonText:{displayText: `💸PAYMENT`},type:1}]
              imageMsg = (await Lazuardi.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created Subscribe RUIN FF', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await Lazuardi.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
...
• Pilih Payment Jika Ingin Membeli ${_diamond1} Dengan Harga ${diamond1}

• Pilih Back Jika Ingin Kembali Ke List Menu`
  buttons = [{buttonId: noprefix ? "store" : "!"+"store",buttonText:{displayText: `⬅️BACK`},type:1},{buttonId: `${prefix}pff1`,buttonText:{displayText: `💸PAYMENT`},type:1}]
              imageMsg = (await Lazuardi.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created Subscribe RUIN FF', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await Lazuardi.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
...
• Pilih Payment Jika Ingin Membeli ${_diamond2} Dengan Harga ${diamond2}

• Pilih Back Jika Ingin Kembali Ke List Menu`
  buttons = [{buttonId: noprefix ? "store" : "!"+"store",buttonText:{displayText: `⬅️BACK`},type:1},{buttonId: `${prefix}pff2`,buttonText:{displayText: `💸PAYMENT`},type:1}]
              imageMsg = (await Lazuardi.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created Subscribe RUIN FF', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await Lazuardi.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
...
• Pilih Payment Jika Ingin Membeli ${_diamond3} Dengan Harga ${diamond3}

• Pilih Back Jika Ingin Kembali Ke List Menu`
  buttons = [{buttonId: noprefix ? "store" : "!"+"store",buttonText:{displayText: `⬅️BACK`},type:1},{buttonId: `${prefix}pff3`,buttonText:{displayText: `💸PAYMENT`},type:1}]
              imageMsg = (await Lazuardi.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created Subscribe RUIN FF', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await Lazuardi.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
...
• Pilih Payment Jika Ingin Membeli ${_diamond4} Dengan Harga ${diamond4}

• Pilih Back Jika Ingin Kembali Ke List Menu`
  buttons = [{buttonId: noprefix ? "store" : "!"+"store",buttonText:{displayText: `⬅️BACK`},type:1},{buttonId: `${prefix}pff4`,buttonText:{displayText: `💸PAYMENT`},type:1}]
              imageMsg = (await Lazuardi.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created Subscribe RUIN FF', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await Lazuardi.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
...
• Pilih Payment Jika Ingin Membeli ${_diamond5} Dengan Harga ${diamond5}

• Pilih Back Jika Ingin Kembali Ke List Menu`
  buttons = [{buttonId: noprefix ? "store" : "!"+"store",buttonText:{displayText: `⬅️BACK`},type:1},{buttonId: `${prefix}pff5`,buttonText:{displayText: `💸PAYMENT`},type:1}]
              imageMsg = (await Lazuardi.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created Subscribe RUIN FF', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await Lazuardi.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
...
• Pilih Payment Jika Ingin Membeli ${_diamond6} Dengan Harga ${diamond6}

• Pilih Back Jika Ingin Kembali Ke List Menu`
  buttons = [{buttonId: noprefix ? "store" : "!"+"store",buttonText:{displayText: `⬅️BACK`},type:1},{buttonId: `${prefix}pff6`,buttonText:{displayText: `💸PAYMENT`},type:1}]
              imageMsg = (await Lazuardi.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created Subscribe RUIN FF', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await Lazuardi.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
...
• Pilih Payment Jika Ingin Membeli ${_diamond7} Dengan Harga ${diamond7}

• Pilih Back Jika Ingin Kembali Ke List Menu`
  buttons = [{buttonId: noprefix ? "store" : "!"+"store",buttonText:{displayText: `⬅️BACK`},type:1},{buttonId: `${prefix}pff7`,buttonText:{displayText: `💸PAYMENT`},type:1}]
              imageMsg = (await Lazuardi.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created Subscribe RUIN FF', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await Lazuardi.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
...
• Pilih Payment Jika Ingin Membeli ${_diamond8} Dengan Harga ${diamond8}

• Pilih Back Jika Ingin Kembali Ke List Menu`
  buttons = [{buttonId: noprefix ? "store" : "!"+"store",buttonText:{displayText: `⬅️BACK`},type:1},{buttonId: `${prefix}pff8`,buttonText:{displayText: `💸PAYMENT`},type:1}]
              imageMsg = (await Lazuardi.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created Subscribe RUIN FF', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await Lazuardi.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
...
• Pilih Payment Jika Ingin Membeli ${_diamond9} Dengan Harga ${diamond9}

• Pilih Back Jika Ingin Kembali Ke List Menu`
  buttons = [{buttonId: noprefix ? "store" : "!"+"store",buttonText:{displayText: `⬅️BACK`},type:1},{buttonId: `${prefix}pff9`,buttonText:{displayText: `💸PAYMENT`},type:1}]
              imageMsg = (await Lazuardi.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created Subscribe RUIN FF', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await Lazuardi.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
...
• Pilih Payment Jika Ingin Membeli ${_diamond10} Dengan Harga ${diamond10}

• Pilih Back Jika Ingin Kembali Ke List Menu`
  buttons = [{buttonId: noprefix ? "store" : "!"+"store",buttonText:{displayText: `⬅️BACK`},type:1},{buttonId: `${prefix}pff10`,buttonText:{displayText: `💸PAYMENT`},type:1}]
              imageMsg = (await Lazuardi.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created Subscribe RUIN FF', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await Lazuardi.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
...
• Pilih Payment Jika Ingin Membeli ${_diamond11} Dengan Harga ${diamond11}

• Pilih Back Jika Ingin Kembali Ke List Menu`
  buttons = [{buttonId: noprefix ? "store" : "!"+"store",buttonText:{displayText: `⬅️BACK`},type:1},{buttonId: `${prefix}pff11`,buttonText:{displayText: `💸PAYMENT`},type:1}]
              imageMsg = (await Lazuardi.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created Subscribe RUIN FF', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await Lazuardi.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
...
• Pilih Payment Jika Ingin Membeli ${_diamond12} Dengan Harga ${diamond12}

• Pilih Back Jika Ingin Kembali Ke List Menu`
  buttons = [{buttonId: noprefix ? "store" : "!"+"store",buttonText:{displayText: `⬅️BACK`},type:1},{buttonId: `${prefix}pff12`,buttonText:{displayText: `💸PAYMENT`},type:1}]
              imageMsg = (await Lazuardi.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created Subscribe RUIN FF', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await Lazuardi.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
...
• Pilih Payment Jika Ingin Membeli ${_diamond13} Dengan Harga ${diamond13}

• Pilih Back Jika Ingin Kembali Ke List Menu`
  buttons = [{buttonId: noprefix ? "store" : "!"+"store",buttonText:{displayText: `⬅️BACK`},type:1},{buttonId: `${prefix}pff13`,buttonText:{displayText: `💸PAYMENT`},type:1}]
              imageMsg = (await Lazuardi.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created Subscribe RUIN FF', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await Lazuardi.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
              Lazuardi.relayWAMessage(prep)
      break
/////mllll game ga logisss :v
case 'payml1':
              gopeynya = `${ppbot}`
            buff = await getBuffer(gopeynya)
            teksnya = `*${ucapanWaktu} ${pushname} 👋*
            
• Pilih Payment Jika Ingin Membeli ${_ml1} Dengan Harga ${ml1}

• Pilih Back Jika Ingin Kembali Ke List Menu`
  buttons = [{buttonId: noprefix ? "store" : "!"+"store",buttonText:{displayText: `⬅️BACK`},type:1},{buttonId: `${prefix}pml1`,buttonText:{displayText: `💸PAYMENT`},type:1}]
              imageMsg = (await Lazuardi.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created Subscribe RUIN FF', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await Lazuardi.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
...
• Pilih Payment Jika Ingin Membeli ${_ml2} Dengan Harga ${ml2}

• Pilih Back Jika Ingin Kembali Ke List Menu`
  buttons = [{buttonId: noprefix ? "store" : "!"+"store",buttonText:{displayText: `⬅️BACK`},type:1},{buttonId: `${prefix}pml2`,buttonText:{displayText: `💸PAYMENT`},type:1}]
              imageMsg = (await Lazuardi.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created Subscribe RUIN FF', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await Lazuardi.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
...
• Pilih Payment Jika Ingin Membeli ${_ml3} Dengan Harga ${ml3}

• Pilih Back Jika Ingin Kembali Ke List Menu`
  buttons = [{buttonId: noprefix ? "store" : "!"+"store",buttonText:{displayText: `⬅️BACK`},type:1},{buttonId: `${prefix}pml3`,buttonText:{displayText: `💸PAYMENT`},type:1}]
              imageMsg = (await Lazuardi.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created Subscribe RUIN FF', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await Lazuardi.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
...
• Pilih Payment Jika Ingin Membeli ${_ml4} Dengan Harga ${ml4}

• Pilih Back Jika Ingin Kembali Ke List Menu`
  buttons = [{buttonId: noprefix ? "store" : "!"+"store",buttonText:{displayText: `⬅️BACK`},type:1},{buttonId: `${prefix}pml4`,buttonText:{displayText: `💸PAYMENT`},type:1}]
              imageMsg = (await Lazuardi.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created Subscribe RUIN FF', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await Lazuardi.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
...
• Pilih Payment Jika Ingin Membeli ${_ml5} Dengan Harga ${ml5}

• Pilih Back Jika Ingin Kembali Ke List Menu`
  buttons = [{buttonId: noprefix ? "store" : "!"+"store",buttonText:{displayText: `⬅️BACK`},type:1},{buttonId: `${prefix}pml5`,buttonText:{displayText: `💸PAYMENT`},type:1}]
              imageMsg = (await Lazuardi.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created Subscribe RUIN FF', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await Lazuardi.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
...
• Pilih Payment Jika Ingin Membeli ${_ml6} Dengan Harga ${ml6}

• Pilih Back Jika Ingin Kembali Ke List Menu`
  buttons = [{buttonId: noprefix ? "store" : "!"+"store",buttonText:{displayText: `⬅️BACK`},type:1},{buttonId: `${prefix}pml6`,buttonText:{displayText: `💸PAYMENT`},type:1}]
              imageMsg = (await Lazuardi.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created Subscribe RUIN FF', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await Lazuardi.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
...
• Pilih Payment Jika Ingin Membeli ${_ml7} Dengan Harga ${ml7}

• Pilih Back Jika Ingin Kembali Ke List Menu`
  buttons = [{buttonId: noprefix ? "store" : "!"+"store",buttonText:{displayText: `⬅️BACK`},type:1},{buttonId: `${prefix}pml7`,buttonText:{displayText: `💸PAYMENT`},type:1}]
              imageMsg = (await Lazuardi.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created Subscribe RUIN FF', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await Lazuardi.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
...
• Pilih Payment Jika Ingin Membeli ${_ml8} Dengan Harga ${ml8}

• Pilih Back Jika Ingin Kembali Ke List Menu`
  buttons = [{buttonId: noprefix ? "store" : "!"+"store",buttonText:{displayText: `⬅️BACK`},type:1},{buttonId: `${prefix}pml8`,buttonText:{displayText: `💸PAYMENT`},type:1}]
              imageMsg = (await Lazuardi.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created Subscribe RUIN FF', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await Lazuardi.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
...
• Pilih Payment Jika Ingin Membeli ${_ml9} Dengan Harga ${ml9}

• Pilih Back Jika Ingin Kembali Ke List Menu`
  buttons = [{buttonId: noprefix ? "store" : "!"+"store",buttonText:{displayText: `⬅️BACK`},type:1},{buttonId: `${prefix}pml9`,buttonText:{displayText: `💸PAYMENT`},type:1}]
              imageMsg = (await Lazuardi.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created Subscribe RUIN FF', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await Lazuardi.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
...
• Pilih Payment Jika Ingin Membeli ${_ml10} Dengan Harga ${ml10}

• Pilih Back Jika Ingin Kembali Ke List Menu`
  buttons = [{buttonId: noprefix ? "store" : "!"+"store",buttonText:{displayText: `⬅️BACK`},type:1},{buttonId: `${prefix}pml10`,buttonText:{displayText: `💸PAYMENT`},type:1}]
              imageMsg = (await Lazuardi.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created Subscribe RUIN FF', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await Lazuardi.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
...
• Pilih Payment Jika Ingin Membeli ${_ml11} Dengan Harga ${ml11}

• Pilih Back Jika Ingin Kembali Ke List Menu`
  buttons = [{buttonId: noprefix ? "store" : "!"+"store",buttonText:{displayText: `⬅️BACK`},type:1},{buttonId: `${prefix}pml11`,buttonText:{displayText: `💸PAYMENT`},type:1}]
              imageMsg = (await Lazuardi.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created Subscribe RUIN FF', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await Lazuardi.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
...
• Pilih Payment Jika Ingin Membeli ${_ml12} Dengan Harga ${ml12}

• Pilih Back Jika Ingin Kembali Ke List Menu`
  buttons = [{buttonId: noprefix ? "store" : "!"+"store",buttonText:{displayText: `⬅️BACK`},type:1},{buttonId: `${prefix}pml12`,buttonText:{displayText: `💸PAYMENT`},type:1}]
              imageMsg = (await Lazuardi.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created Subscribe RUIN FF', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await Lazuardi.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
...
• Pilih Payment Jika Ingin Membeli ${_ml13} Dengan Harga ${ml13}

• Pilih Back Jika Ingin Kembali Ke List Menu`
  buttons = [{buttonId: noprefix ? "store" : "!"+"store",buttonText:{displayText: `⬅️BACK`},type:1},{buttonId: `${prefix}pml13`,buttonText:{displayText: `💸PAYMENT`},type:1}]
              imageMsg = (await Lazuardi.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created Subscribe RUIN FF', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await Lazuardi.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
...
case 'pff1':
               list = []
               listmenu = [`ffgopay1`,`ffdana1`,`ffovo1`]
               listmenuu = [`💸GOPAY`,`💸DANA`,`💸OVO`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
               const yy = {title: 'menu ke' + nombor++,
...
Silahkan Pilih Metode Pembayaranya Di Sini
`,`🐣 Day : ${week} ${weton}\n🐥 Date : ${date}              `, list, { quoted: freply})
               break
case 'pff2':
               list = []
               listmenu = [`ffgopay2`,`ffdana2`,`ffovo2`]
               listmenuu = [`💸GOPAY`,`💸DANA`,`💸OVO`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
...
case 'pff3':
               list = []
               listmenu = [`ffgopay3`,`ffdana3`,`ffovo3`]
               listmenuu = [`💸GOPAY`,`💸DANA`,`💸OVO`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
...
case 'pff4':
               list = []
               listmenu = [`ffgopay4`,`ffdana4`,`ffovo4`]
               listmenuu = [`💸GOPAY`,`💸DANA`,`💸OVO`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
...
case 'pff5':
               list = []
               listmenu = [`ffgopay5`,`ffdana5`,`ffovo5`]
               listmenuu = [`💸GOPAY`,`💸DANA`,`💸OVO`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
...
case 'pff6':
               list = []
               listmenu = [`ffgopay6`,`ffdana6`,`ffovo6`]
               listmenuu = [`💸GOPAY`,`💸DANA`,`💸OVO`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
...
case 'pff7':
               list = []
               listmenu = [`ffgopay7`,`ffdana7`,`ffovo7`]
               listmenuu = [`💸GOPAY`,`💸DANA`,`💸OVO`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
...
case 'pff8':
               list = []
               listmenu = [`ffgopay8`,`ffdana8`,`ffovo8`]
               listmenuu = [`💸GOPAY`,`💸DANA`,`💸OVO`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
...
case 'pff9':
               list = []
               listmenu = [`ffgopay9`,`ffdana9`,`ffovo9`]
               listmenuu = [`💸GOPAY`,`💸DANA`,`💸OVO`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
...
case 'pff10':
               list = []
               listmenu = [`ffgopay10`,`ffdana10`,`ffovo10`]
               listmenuu = [`💸GOPAY`,`💸DANA`,`💸OVO`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
...
case 'pff11':
               list = []
               listmenu = [`ffgopay11`,`ffdana11`,`ffovo11`]
               listmenuu = [`💸GOPAY`,`💸DANA`,`💸OVO`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
...
case 'pff12':
               list = []
               listmenu = [`ffgopay12`,`ffdana12`,`ffovo12`]
               listmenuu = [`💸GOPAY`,`💸DANA`,`💸OVO`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
...
case 'pff13':
               list = []
               listmenu = [`ffgopay13`,`ffdana13`,`ffovo13`]
               listmenuu = [`💸GOPAY`,`💸DANA`,`💸OVO`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
...
case 'pml1':
               list = []
               listmenu = [`mlgopay1`,`mldana1`,`mlovo1`]
               listmenuu = [`💸GOPAY`,`💸DANA`,`💸OVO`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
...
case 'pml2':
               list = []
               listmenu = [`mlgopay2`,`mldana2`,`mlovo2`]
               listmenuu = [`💸GOPAY`,`💸DANA`,`💸OVO`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
...
case 'pml3':
               list = []
               listmenu = [`mlgopay3`,`mldana3`,`mlovo3`]
               listmenuu = [`💸GOPAY`,`💸DANA`,`💸OVO`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
...
case 'pml4':
               list = []
               listmenu = [`mlgopay4`,`mldana4`,`mlovo4`]
               listmenuu = [`💸GOPAY`,`💸DANA`,`💸OVO`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
...
case 'pml5':
               list = []
               listmenu = [`mlgopay5`,`mldana5`,`mlovo5`]
               listmenuu = [`💸GOPAY`,`💸DANA`,`💸OVO`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
...
case 'pml6':
               list = []
               listmenu = [`mlgopay6`,`mldana6`,`mlovo6`]
               listmenuu = [`💸GOPAY`,`💸DANA`,`💸OVO`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
...
case 'pml7':
               list = []
               listmenu = [`mlgopay7`,`mldana7`,`mlovo7`]
               listmenuu = [`💸GOPAY`,`💸DANA`,`💸OVO`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
...
case 'pml8':
               list = []
               listmenu = [`mlgopay8`,`mldana8`,`mlovo8`]
               listmenuu = [`💸GOPAY`,`💸DANA`,`💸OVO`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
...
case 'pml9':
               list = []
               listmenu = [`mlgopay9`,`mldana9`,`mlovo9`]
               listmenuu = [`💸GOPAY`,`💸DANA`,`💸OVO`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
...
case 'pml10':
               list = []
               listmenu = [`mlgopay10`,`mldana10`,`mlovo10`]
               listmenuu = [`💸GOPAY`,`💸DANA`,`💸OVO`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
...
case 'pml11':
               list = []
               listmenu = [`mlgopay11`,`mldana11`,`mlovo11`]
               listmenuu = [`💸GOPAY`,`💸DANA`,`💸OVO`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
...
case 'pml12':
               list = []
               listmenu = [`mlgopay12`,`mldana12`,`mlovo12`]
               listmenuu = [`💸GOPAY`,`💸DANA`,`💸OVO`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
...
case 'pml13':
               list = []
               listmenu = [`mlgopay13`,`mldana13`,`mlovo13`]
               listmenuu = [`💸GOPAY`,`💸DANA`,`💸OVO`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
...
case 'ppubg1':
               list = []
               listmenu = [`pubggopay1`,`pubgdana1`,`pubgovo1`]
               listmenuu = [`💸GOPAY`,`💸DANA`,`💸OVO`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
...
case 'ppubg2':
               list = []
               listmenu = [`pubggopay2`,`pubgdana2`,`pubgovo2`]
               listmenuu = [`💸GOPAY`,`💸DANA`,`💸OVO`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
...
case 'ppubg3':
               list = []
               listmenu = [`pubggopay3`,`pubgdana3`,`pubgovo3`]
               listmenuu = [`💸GOPAY`,`💸DANA`,`💸OVO`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
...
case 'ppubg4':
               list = []
               listmenu = [`pubggopay4`,`pubgdana4`,`pubgovo4`]
               listmenuu = [`💸GOPAY`,`💸DANA`,`💸OVO`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
...
case 'ppubg5':
               list = []
               listmenu = [`pubggopay5`,`pubgdana5`,`pubgovo5`]
               listmenuu = [`💸GOPAY`,`💸DANA`,`💸OVO`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
...
case 'ppubg6':
               list = []
               listmenu = [`pubggopay6`,`pubgdana6`,`pubgovo6`]
               listmenuu = [`💸GOPAY`,`💸DANA`,`💸OVO`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
...
case 'ppubg7':
               list = []
               listmenu = [`pubggopay7`,`pubgdana7`,`pubgovo7`]
               listmenuu = [`💸GOPAY`,`💸DANA`,`💸OVO`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
...
case 'ppubg8':
               list = []
               listmenu = [`pubggopay8`,`pubgdana8`,`pubgovo8`]
               listmenuu = [`💸GOPAY`,`💸DANA`,`💸OVO`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
...
case 'ppubg9':
               list = []
               listmenu = [`pubggopay9`,`pubgdana9`,`pubgovo9`]
               listmenuu = [`💸GOPAY`,`💸DANA`,`💸OVO`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
...
case 'ppubg10':
               list = []
               listmenu = [`pubggopay10`,`pubgdana10`,`pubgovo10`]
               listmenuu = [`💸GOPAY`,`💸DANA`,`💸OVO`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
...
case 'ppubg11':
               list = []
               listmenu = [`pubggopay11`,`pubgdana11`,`pubgovo11`]
               listmenuu = [`💸GOPAY`,`💸DANA`,`💸OVO`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
...
case 'ppubg12':
               list = []
               listmenu = [`pubggopay12`,`pubgdana12`,`pubgovo12`]
               listmenuu = [`💸GOPAY`,`💸DANA`,`💸OVO`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
...
case 'ppubg13':
               list = []
               listmenu = [`pubggopay13`,`pubgdana13`,`pubgovo13`]
               listmenuu = [`💸GOPAY`,`💸DANA`,`💸OVO`]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
...
•••••••••••••••••••••
• • _Nominal : ${_diamond1}_
• • _Harga : ${diamond1}_
•••••••••••••••••••••
• _*${date}*_
•••••••••••••••••••••

*Kirim Id Dengan Cara Ketik _Contoh : !id 123456789_*`
  buttons = [{buttonId: noprefix ? "owner" : "!"+"owner",buttonText:{displayText: `📝KIRIM ID NYA KAK📝`},type:1}]
              imageMsg = (await Lazuardi.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'SUBSCRIBE RUIN FF', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await Lazuardi.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
              Lazuardi.relayWAMessage(prep)
      break
...
*Kirim Id Dengan Cara Ketik _Contoh : !id 123456789_*`
  buttons = [{buttonId: noprefix ? "owner" : "!"+"owner",buttonText:{displayText: `📝KIRIM ID NYA KAK📝`},type:1}]
      ...
      break
case 'id':
              if (args.length == 0) return reply(`Example: ${prefix + command} 123456789`)
...
⬇️Silahkan Kirim Bukti Transfer Ke Owner
`,`🐣 Day : ${week} ${weton}\n🐥 Date : ${date}              `, list, { quoted: freply})
               break
...
*「 RUIN FF 」*`
	    ...
	    // console.log(e)
            setTimeout( () => {
			Lazuardi.updatePresence(from, Presence.composing)
			reply(`*Terimakasih Telah Daftar, Subscribe RUIN FF*`)
		}, 2000)
        break
case 'sfire1':{
...
📱GOPAY : _${nogopay}_\n📱DANA : _${nodana}_\n📱OVO : _${noovo}_`
  buttons = [{buttonId: noprefix ? "menu" : "!"+"sewa1",buttonText:{displayText: `OWNER`},type:1}]
              imageMsg = (await Lazuardi.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created Subscribe RUIN FF', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await Lazuardi.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
...
Wa.me/6283141727903`
   sendButMessage(from, titid, `Created Subscribe RUIN FF `, [
          {
...
"productId": "4120139628109348"
  ...
  case 'scan':
    ...
    "listMessage": {
      ...
      "footerText": "\nPowered Created Subscribe RUIN FF.                                        "
    }
  }, {quoted:kyy})
  Lazuardi.relayWAMessage(list, {waitForAck: true})
  break
...
//------------------< Ingfo Bot >-------------------
      case 'runtime':
       ...
       case 'sc': 
       case 'src':
              textImg(`Bot ini menggunakan sc : https://m.youtube.com/channel/UCqkpkST3ElnoHNAwEcuRX3g`)
              sendKontak(from, `6285855159230`, `Developer`, 'Sc jgn di jual!')
              break
      case 'ping':
      case 'speed':
              timestampe = speed();
...
//------------------< Owner >-------------------
      ...
      case 'bcbutton':
			...
			"footerText": 'Created Subscribe RUIN FF☕︎',
			"buttons": [{ buttonId: noprefix ? "menu" : "!"+"menu", buttonText: { displayText: '🏷️LIST MENU' }, type: 1 },{ buttonId: noprefix ? "store" : "!"+"store", buttonText: { displayText: '🛒STORE' }, type: 1 },{ buttonId: noprefix ? "sewa" : "!"+"sewa", buttonText: { displayText: '📝SEWA' }, type: 1 } ], headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: ikyads, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')
}
             reply('Suksess broadcast')
}
             break
...
case 'caripesan':  //by ANU TEAM
       ...
       case 'bugreport':
              ...
              Lazuardi.sendMessage('6285855159230@s.whatsapp.net',`*Bug Report:* ${teks}`, text)
              break
       case 'readall':
...
//------------------< Menunya Bang:v >-------------------
      case 'infoig':
             reply(`Jangan Lupa Follow Ig Owner Ya : https://www.instagram.com/ardiwj13/`)
             break
      case 'Lazuardigroup':
             reply('https://chat.whatsapp.com/CTqGZl1iQaDKeecDOEOasr')
             break
